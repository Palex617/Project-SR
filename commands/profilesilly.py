...
class ProfileSilly(commands.Cog):
    ...
    async def on_ready(self):
        print('ProfileSilly cog loaded.')
    
    @app_commands.command(name='profilesilly', description='Enter UID')
    async def questions(self, interaction:discord.Interaction, uid:str):
        ...
        async with client:
            ...
            charDictSilly = dictionaries.characterInfo.charDictSilly
            ...
            embed.set_thumbnail(url=f"{charDict.get(data.player.avatar.id).get('icon')}")
            ...
            characterList = ""
            charName = charDictSilly.get(data.player.characters_preview[0].name)
            ...
            for char in range(len(data.player.characters_preview)):
                ...
                charName = charDictSilly.get(data.player.characters_preview[char].name)
            ...
            async def charDetails(interaction):
                ...
                # This gives error if there are no characters in showcase
                ...
                charEmbed = discord.Embed(title=f"{data.player.nickname}'s C{cons} {charName}",
                    ...
                    color=embedcolor)
                charEmbed.set_thumbnail(url=f"{charDict.get(charID).get('icon')}")
...
async def setup(bot):
    await bot.add_cog(ProfileSilly(bot))